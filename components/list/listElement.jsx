...
const ListElement = ({ data, description, width, value, index, error, errorMessage, warning, warningMessage }) => {
    return (
        <div
            className={`flex items-center space-x-8 lg:space-x-8 p-2 text-sm font-body textColor ${
                index % 2 == 0 ? "" : "bg-primaryColor-100"
            }`}
        >
            <div
                className={`left w-1/6 textColor lg:w-1/6 font-semibold flex text-xs lg:text-sm items-center ${width}`}
            >
                {description}
            </div>
            <div
                className={`right space-x-2 flex textColor items-center text-xs lg:text-sm text-successColor ${
            ...
            >
                ...
                <p className="text-xs lg:text-sm textColor">
