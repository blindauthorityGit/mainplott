...
export default function ChoiceCard({ icon: Icon, heading, description, configuratorValue }) {
    ...
    return (
        <div className="relative w-full flex-1 flex flex-col">
            ...
            {/* Main Content */}
            <motion.div
                className={`relative w-full h-full lg:p-8 rounded-lg border-2 border-textColor flex flex-col justify-between ${
            ...
            >
                ...
                {/* Heading */}
                <H4 klasse="text-xl font-bold text-textColor !text-center mb-2">{heading}</H4>

                {/* Description */}
                <P klasse="text-center">{description}</P>
            </motion.div>
        </div>
    );
}
